...
<button-modal
            ...
            modal-local-component="FormDynamic"
            :comp-params="{
...
export default {
  name: 'JustButtonModal',
  components: {
  },
  props: {
    modalLocalComponent: { // Название компонента передано локально
      type: String,
      default: null
    },
    modalLocalComponentParams: { // Передача параметров в компонент локально
      type: [Array, Object],
      default: {}
    },
    compParams: {
  ...
  methods: {
    ...
    openModal(){
      ...
      this.setModalClass(this.compParams.modalClass ? this.compParams.modalClass : ''); // класс для модального окна
      this.setModalComponentClass(this.compParams.modalComponentClass ? this.compParams.modalComponentClass : ''); // класс для компонента
